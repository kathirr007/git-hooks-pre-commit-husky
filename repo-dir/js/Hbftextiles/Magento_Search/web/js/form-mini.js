...
define([
  ...
  $.widget('mage.quickSearch', {
    ...
    _onPropertyChange: function () {
      ...
      const source = this.options.template
      const template = mageTemplate(source)
      const dropdown = $('<ul role="listbox"></ul>')
      let value = this.element.val()
      value = value.replace(/[^0-9a-zA-Z_ \.]/g, '')
      ...
      if (value.length >= parseInt(this.options.minSearchLength, 10)) {
        $.getJSON(
          ...
          $.proxy(function (data) {
            ...
            if (data.length) {
              let result_data = true
              $.each(data, function (index, element) {
                // let html;

                if (element.num_results == '') {
                  result_data = false
                }
              ...
              if (result_data) {
                $('#search_autocomplete').removeClass(
                  'empty-search-result'
                )
                $('#search_autocomplete ul').show()
